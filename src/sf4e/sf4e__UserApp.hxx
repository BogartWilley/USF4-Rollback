...
namespace sf4e {
    struct UserApp : Dimps::UserApp
    {
        ...
        static void StartClient(char* joinAddr, uint16_t port, std::string& name, uint8_t deviceType, uint8_t deviceIdx, uint8_t delay);
        static void StartServer(uint16 hostPort);
    };
}